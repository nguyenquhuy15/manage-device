...
const DepartmentPage = () => {
  ...
  const existsByManagerName = useCallback(async (managerName) => {
    return await DepartmentApi.existsByManagerName(managerName);
  }, []);

  const existsByEmail = useCallback(async (email) => {
    return await DepartmentApi.existsByEmail(email);
  }, []);
  ...
  const onCreateDepartment = useCallback(
    async (
      ...
      // show notification
      showSuccessMessage("Tạo phòng thí nghiệm thành công!");
      // hide modal
      hideCreateDepartmentModal();
  ...
  return (
    <>
      <Helmet title="Khoa/Viện" />
      <Container fluid className="p-0">
        <h1 className="h3 mb-3">Thống kê Khoa/Viện & Bộ Môn</h1>

        {departments.length == 0 && isSearchEmpty() ? (
        ...
        {isShowCreateDepartmentModal && (
          <CreateDepartmentModal
            ...
            existsByManagerName={existsByManagerName}

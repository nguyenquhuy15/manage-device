import React, { useCallback, useEffect, useState } from "react";
import { Helmet } from "react-helmet-async";
import useBoolean from "../../hooks/useBoolean";
import { Card, Container } from "react-bootstrap";

import useNotification from "../../hooks/useNotification";
import AccountApi from "../../api/AccountApi";

import DeleteAccountModal from "../../components/accounts/remove/DeleteAccountModal";
import AccountTableComponent from "../../components/accounts/viewlist/AccountTableComponent";
import AccountNoDataComponent from "../../components/accounts/viewlist/AccountNoDataComponent";
import CreateAccountModal from "../../components/accounts/create/CreateAccountModal";
import UserApi from "../../api/UserApi";


const AccountPage = () => {
  const [showSuccessMessage, showErrorMessage] = useNotification();
  const [accounts, setAccounts] = useState([]);
  // paging
  const [currentPage, setCurrentPage] = useState(1);
  const [totalPages, setTotalPages] = useState(0);
  const resetPaging = useCallback(() => setCurrentPage(1), []);
  // sorting
  const [currentSort, setCurrentSort] = useState({
    sortField: "id",
    isAsc: false,
  });
  const resetCurrentSort = useCallback(
    () => setCurrentSort({ sortField: "id", isAsc: false }),
    []
  );
  // search
  const [currentSearch, setCurrentSearch] = useState("");
  const resetCurrentSearch = useCallback(() => {
    setCurrentSearch("");
  }, []);
  const isSearchEmpty = () => {
    return !currentSearch || currentSearch.length === 0;
  };
  const [currentFilter, setCurrentFilter] = useState({
    role: "",
    status: ""
  });

  // refresh table
  const [timeRefreshTable, setTimeRefreshTable] = useState(new Date());
  //create
    const existsByUsername = useCallback(async (username) => {
      return await UserApi.existsByUsername(username);
    }, []);

    const existsByEmail = useCallback(async (email) => {
      return await UserApi.existsByEmail(email);
    }, []);
  
  const [
    isShowCreateAccountModal,
    showCreateAccountModal,
    hideCreateAccountModal,
  ] = useBoolean(false);
    const [timeRefreshCreateForm, setTimeRefreshCreateForm] = useState(
      new Date()
  );
  
    const onShowCreateAccountModal = () => {
      // reset modal
      setTimeRefreshCreateForm(new Date());
      // show modal
      showCreateAccountModal();
  };
  
  const onCreateAccount = useCallback(
    async (firstname, lastname, username, email, password, status) => {
      // call api
      await AccountApi.create(firstname, lastname, username, email, password, status);
      // show notification
      showSuccessMessage("Tạo tài khoản thành công!");
      // hide modal
      hideCreateAccountModal();
      // reset account table
      onResetTable();
    }
  );

  // delete
  const [
    isShowDeleteAccountModal,
    showDeleteAccountModal,
    hideDeleteAccountModal,
  ] = useBoolean(false);

  const [deletingAccountId, setDeletingAccountId] = useState();

  useEffect(() => {
    getListAccounts();
  }, [currentPage, currentSort, currentSearch, currentFilter]);

  const getListAccounts = async () => {
    const data = await AccountApi.getAll({
      page: currentPage,
      sortField: currentSort.sortField,
      isAsc: currentSort.isAsc,
      search: currentSearch,
      role: currentFilter.role,
      status: currentFilter.status,
    });
    setAccounts(data.content);
    setTotalPages(data.totalPages);
  };
  const resetCurrentFilter = useCallback(() => {
    setCurrentFilter({
      role: "",
      status: "",
    });
  }, []);
  const isFilterEmpty = () => {
    return !currentFilter.role && !currentFilter.status;
  };

  const onResetTable = useCallback(() => {
    resetPaging();
    resetCurrentSort();
    resetCurrentSearch();
    resetCurrentFilter();
    setTimeRefreshTable(new Date());
    setDeletingAccountId(undefined);
  }, []);
  const onShowDeleteAccountModal = useCallback((accountId) => {
    showDeleteAccountModal();
    setDeletingAccountId(accountId);
  }, []);

  const onDeleteAccount = useCallback(async () => {
    await AccountApi.delete(deletingAccountId);
    showSuccessMessage("Xóa tài khoản thành công!");
    hideDeleteAccountModal();
    onResetTable();
  }, [deletingAccountId]);
  return (
    <>
      <Helmet title="Account" />
      <Container fluid className="p-0">
        <h1 className="h3 mb-3">Q</h1>

        {accounts.length == 0 && isSearchEmpty() && isFilterEmpty() ? (
          <AccountNoDataComponent />
        ) : (
          <AccountTableComponent
            // table
            accounts={accounts}
            // paging
            totalPages={totalPages}
            currentPage={currentPage}
            setCurrentPage={setCurrentPage}
            resetPaging={resetPaging}
            // sorting
            currentSort={currentSort}
            setCurrentSort={setCurrentSort}
            resetCurrentSort={resetCurrentSort}
            // search
            currentSearch={currentSearch}
            setCurrentSearch={setCurrentSearch}
            // filter
            currentFilter={currentFilter}
            setCurrentFilter={setCurrentFilter}
            // refresh table
            timeRefreshTable={timeRefreshTable}
            onResetTable={onResetTable}
            //onClick
            onCreate={onShowCreateAccountModal}
            onDeleteItem={onShowDeleteAccountModal}
          />
        )}
        {isShowCreateAccountModal && (
          <CreateAccountModal
            hideModal={hideCreateAccountModal}
            timeRefreshForm={timeRefreshCreateForm}
            existsByUsername={existsByUsername}
            existsByEmail={existsByEmail}
            onSubmit={onCreateAccount}
          />
        )}
        {isShowDeleteAccountModal && (
          <DeleteAccountModal
            hideModal={hideDeleteAccountModal}
            name={
              accounts.find((account) => account.id === deletingAccountId).name
            }
            onSubmit={onDeleteAccount}
          />
        )}
      </Container>
    </>
  );
};

export default AccountPage;

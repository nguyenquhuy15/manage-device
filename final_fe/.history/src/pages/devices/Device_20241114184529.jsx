...
const renameOfDeviceKeys = (jsonArray) => {
  jsonArray.forEach((item) => {
    delete Object.assign(item, { deviceName: item["Tên Thiết Bị"] })[
      "Tên Thiết Bị"
    ];
    delete Object.assign(item, { type: item["Loại"] })["Loại"];
    delete Object.assign(item, { price: item["Giá"] })["Giá"];
    delete Object.assign(item, { manager: item["Người Quản Lý"] })[
      "Người Quản Lý"
    ];
    delete Object.assign(item, { status: item["Trạng Thái"] })["Trạng Thái"];
    delete Object.assign(item, { status: item["Trạng Thái"] })["Phòng"];
  });
};
  ...
  const getMoreDeviceInfo = async (
    ...
    try {
      const data = await DeviceApi.getAll(
        devices.map((device) => device.username)
      );
      for (const item of data) {
        let device = devices.find((device) => device.username == item.username);
        device.id = item.id;

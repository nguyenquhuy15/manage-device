...
const renameOfDeviceKeys = (jsonArray) => {
  ...
  const getMoreDeviceInfo = async (
    ...
    try {
      const data = await DeviceApi.getInfoDevicesByName(devices.map((device) => device.name));

      for (const item of data) {
        let device = devices.find((device) => device.name === item.name);
          // Cập nhật các trường từ API
          device.code = item.code;
          device.name = item.name;
          device.typeName = item.typeName;
          device.price = item.price;
          device.managersName = item.managersName;
          device.status = item.status;
          device.laboratoriesName = item.laboratoriesName;
      }
  ...
  const onImportDevices = useCallback(
    async (code, name, typeId, price, managersId, status, laboratoriesId) => {
      // call api

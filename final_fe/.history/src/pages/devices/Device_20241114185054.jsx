...
const renameOfDeviceKeys = (jsonArray) => {
  jsonArray.forEach((item) => {
    ...
    delete Object.assign(item, { type: item["Loại"] })["Loại"];
    ...
    try {
      ...
      for (const item of data) {
        let device = devices.find(
          (device) => device.deviceName === item.deviceName
        );
        if (device) {
          // Cập nhật các trường từ API
          device.code = item.id;
          device.fullname = item.fullname;
          device.type = item.type; // Giả sử API trả về trường này
          device.price = item.price; // Giả sử API trả về trường này
          device.manager = item.manager; // Giả sử API trả về trường này
          device.status = item.status; // Giả sử API trả về trường này
          // Thêm bất kỳ trường nào khác mà bạn cần cập nhật
        }
      }
  ...
  const onImportDevices = useCallback(
    async (deviceId, managerId, employeeIds) => {
      // call api
      await DeviceApi.importDevices(deviceId, managerId, employeeIds);
      // show notification
      showSuccessMessage("Import Devices Successfully!");
      // hide modal
      hideImportDeviceModal();
      // reset device table
      onResetTable();

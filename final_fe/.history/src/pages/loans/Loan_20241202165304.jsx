...
const LoanPage = () => {
  ...
  const getListLoans = async () => {
    const data = await <LoanApi></LoanApi>.getAll({
      page: currentPage,
  ...
  const renameOfDeviceKeys = (jsonArray) => {
    jsonArray.forEach((item) => {
      ...
      delete Object.assign(item, {
        laboratoriesManagerName: item["Người Quản Lý"],
      })["Người Quản Lý"];
  ...
  const onImportDevices = useCallback(
    async (code, name, typeName, price, laboratoriesManagerName, status) => {
      // call api
  ...
  return (
    <>
      ...
      <Container fluid className="p-0">
        <h1 className="h3 mb-3">Thiết bị</h1>

...
import ReturnLoanModal from "../../components/loans/return/ReturnLoanModal";
...
const LoanPage = () => {
  ...
  // return
 const [isShowReturnLoanModal, setShowReturnLoanModal] = useState(false);
 const [loanIdForReturn, setLoanIdForReturn] = useState();
  const [deviceIdsForReturn, setDeviceIdsForReturn] = useState([]);
  const handleSelectDevice = (selectedDevices) => {
    setDeviceIdsForReturn(selectedDevices); // selectedDevices phải là một mảng
  };

 const handleReturnConfirm = useCallback((loanId, deviceIds) => {
   setLoanIdForReturn(loanId);
   setDeviceIdsForReturn(deviceIds);
   setShowReturnLoanModal(true);
 }, []);

 // Hàm xử lý trả thiết bị
 const handleReturnDevices = useCallback(async () => {
   try {
     await LoanApi.returnLoan(loanIdForReturn, deviceIdsForReturn);
     showSuccessMessage("Thiết bị đã được trả thành công!");
     setShowReturnLoanModal(false);
     onResetTable(); // Làm mới bảng sau khi trả thiết bị
   } catch (error) {
     showErrorMessage(error || "Đã xảy ra lỗi khi trả thiết bị.");
   }
 }, [loanIdForReturn, deviceIdsForReturn]);
      ...
      <Container fluid className="p-0">
        ...
        {loans.length == 0 && isSearchEmpty() && isFilterEmpty() ? (
          ...
          <LoanTableComponent
            ...
            onReturnLoan={handleReturnConfirm}
        ...
        {isShowReturnLoanModal && (
          <ReturnLoanModal
            show={isShowReturnLoanModal}
            onHide={() => setShowReturnLoanModal(false)}
            onSubmit={handleReturnDevices}
            loanId={loanIdForReturn}
            deviceIds={deviceIdsForReturn}
          />
        )}

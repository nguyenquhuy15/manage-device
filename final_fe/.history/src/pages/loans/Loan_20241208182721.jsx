...
const LoanPage = () => {
  ...
  // return
const [isShowReturnLoanModal, showReturnLoanModal, hideReturnLoanModal] =
  useBoolean(false);
const [selectedDeviceIdsForReturn, setSelectedDeviceIdsForReturn] = useState(
  []
);
const [loanIdForReturn, setLoanIdForReturn] = useState(null);

const onReturnLoan = useCallback(async (loanId, deviceIds) => {
  try {
    await LoanApi.returnDevices(loanId, deviceIds); // Call your API endpoint
    showSuccessMessage("Thiết bị đã được trả thành công!");
    hideReturnLoanModal();
    onResetTable(); // Refresh the loan table
  } catch (error) {
    showErrorMessage(
      "Lỗi khi trả thiết bị: " +
        (error.response?.data?.message || error.message)
    );
  }
}, []);

const handleShowReturnLoanModal = (loanId, deviceIds) => {
  setLoanIdForReturn(loanId);
  setSelectedDeviceIdsForReturn(deviceIds); // Pass the deviceIds to the modal
  showReturnLoanModal();
};
      ...
      <Container fluid className="p-0">
        ...
        {loans.length == 0 && isSearchEmpty() && isFilterEmpty() ? (
          ...
          <LoanTableComponent
            ...
            onReturnLoan={handleShowReturnLoanModal}
        ...
        {isShowReturnLoanModal && (
          <ReturnLoanModal
            ...
            onHide={hideReturnLoanModal}
            loanId={loanIdForReturn} // Pass loanId to the modal
            selectedDeviceIds={selectedDeviceIdsForReturn} // Pass deviceIds to the modal

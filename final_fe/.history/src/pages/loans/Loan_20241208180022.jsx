...
const LoanPage = () => {
  ...
  // return
const [isShowReturnLoanModal, setIsShowReturnLoanModal] = useState(false);
const [returningLoanId, setReturningLoanId] = useState(null);
const [selectedDeviceIds, setSelectedDeviceIds] = useState([]);

// Hiển thị modal trả thiết bị
const showReturnLoanModal = (loanId, deviceIds) => {
  setReturningLoanId(loanId);
  setSelectedDeviceIds(deviceIds);
  setIsShowReturnLoanModal(true);
};

// Ẩn modal trả thiết bị
const hideReturnLoanModal = () => {
  setReturningLoanId(null);
  setSelectedDeviceIds([]);
  setIsShowReturnLoanModal(false);
};

// Xử lý trả thiết bị
const onReturnLoan = async (returnDate) => {
  try {
    await LoanApi.returnLoan(returningLoanId, selectedDeviceIds, returnDate); // Gọi API trả thiết bị
    showSuccessMessage("Trả thiết bị thành công!"); // Hiển thị thông báo thành công
    hideReturnLoanModal(); // Đóng modal
    if (onResetTable) onResetTable(); // Làm mới bảng nếu cần
  } catch (error) {
    showErrorMessage("Lỗi khi trả thiết bị."); // Hiển thị thông báo lỗi
  }
};
      ...
      <Container fluid className="p-0">
        ...
        {loans.length == 0 && isSearchEmpty() && isFilterEmpty() ? (
          ...
          <LoanTableComponent
            ...
            onReturnLoan={onShowReturnLoanModal}

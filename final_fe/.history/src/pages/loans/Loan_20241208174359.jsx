...
const LoanPage = () => {
  ...
  const [selectedDeviceIds, setSelectedDeviceIds] = useState([]); // Thêm state để lưu danh sách deviceIds

  const onShowReturnLoanModal = (loanId, deviceIds) => {
    setReturningLoanId(loanId);
    setSelectedDeviceIds(deviceIds); // Lưu danh sách deviceIds khi mở modal
    showReturnLoanModal();
  };

  const onReturnLoan = async (returnDate) => {
    try {
      // Gọi API với loanId và danh sách deviceIds
      await LoanApi.returnLoan(returningLoanId, selectedDeviceIds, returnDate);
      showSuccessMessage("Trả thiết bị thành công!");
      hideReturnLoanModal();
      onResetTable(); // Làm mới bảng sau khi trả thiết bị
    } catch (error) {
      showErrorMessage("Lỗi khi trả thiết bị.");
    }
  };

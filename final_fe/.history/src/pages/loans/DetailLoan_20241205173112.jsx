...
const DetailLoanPage = (props) => {
  ...
  const loansId = props.router.params.id;

  /**
   * Detail loan Info
   */
  const [detailLoanInfo, setDetailDepartmentInfo] = useState({
    name: "",
    location: "",
    departmentsName: "",
    managerName: "",
  ...
  const getDetailDepartmentInfo = async () => {
    try {
      const departmentInfo = await DepartmentApi.getDetail(laboratoriesId);
  ...
  const DetailDepartmentComponentMemo = useMemo(
    () => <DetailDepartmentComponent info={detailDepartmentInfo} />,

...
const onChangeCheckboxItem = (e, deviceId) => {
  // Tạo một bản sao mới của Set
  let addingIds = new Set(props.addingIds);

  if (e.target.checked) {
    addingIds.add(deviceId); // Thêm ID vào Set
  } else {
    addingIds.delete(deviceId); // Xóa ID khỏi Set
  }

  props.setAddingIds(addingIds); // Cập nhật state với Set mới
};

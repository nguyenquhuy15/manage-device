...
import LoanNoDataComponent from "./LoanNoDataComponent";
import AdminContent from "../../../../src/hoc/AdminContent";

const LoanTableComponent = (props) => {
  const TableComponentMemo = useMemo(
    () => (
      <TableComponent
        ...
        onReturnLoan={props.onReturnLoan}
        // onDeleteItem={props.onDeleteItem}
        // removingAllIds={props.removingAllDeviceIds}
        // setRemovingAllIds={props.setRemovingAllIds}
      />
    ),
    [props.loans, props.currentSort]
  );
   const handleReturnClick = (loanId, deviceIds) => {
     props.onReturnLoan(loanId, deviceIds); // Pass loanId and deviceIds to the modal
   };
        ...
        <Card.Body>
          <Row className="mb-3">
            ...
            <Col md={8} className="d-flex flex-row-reverse align-items-center">
              ...
              <OverlayTrigger
                placement="bottom"
                overlay={<Tooltip>Thêm yêu cầu thiết bị</Tooltip>}
              >

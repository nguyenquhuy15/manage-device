...
const TableComponent = React.memo((props) => {
  ...
  const renderReturnButton = (loan) => {
    const deviceIds = loan.devices
      ? loan.devices.map((device) => device.id)
      : []; // xử lý trường hợp loan không có devices
    return (
      <OverlayTrigger
        key="return"
        placement="top"
        overlay={
          <Tooltip id={`tooltip-return-${loan.id}`}>Trả thiết bị</Tooltip>
        }
      >
        <button

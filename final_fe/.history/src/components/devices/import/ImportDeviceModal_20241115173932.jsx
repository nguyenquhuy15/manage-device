...
const handleSubmitForm = async ({
  ...
  try {
    if (props.devices && props.devices.length > 0) {
      ...
      // Kiểm tra thiết bị nào đã tồn tại
      const existingDevices = await checkDevicesExist(deviceCodes);
      ...
      for (const device of props.devices) {
        ...
        if (existingDevices.includes(preparedDevice.code)) {
          ...
          if (!shouldUpdate) {
            continue; // Bỏ qua thiết bị này nếu người dùng không muốn cập nhật
          }
        }
      ...
      // Gọi API để import danh sách thiết bị
      if (devicesToImport.length > 0) {
        await props.onSubmitDevices(devicesToImport);
        showSuccessMessage("Import thiết bị thành công!");
      } else {

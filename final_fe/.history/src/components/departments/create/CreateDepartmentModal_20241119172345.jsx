...
const CreateDepartmentModal = React.memo((props) => {
  ...
  const validationForm = Yup.object({
    ...
    departmentsId: Yup.string().nullable().test(
      "at-least-one",
      "You must select at least one department or subject.",
      function(value) {
        const { subjectsId } = this.parent;
        return value || subjectsId;
      }
    ),
    subjectsId: Yup.string().nullable().test(
      "at-least-one",
      "You must select at least one department or subject.",
      function(value) {
          ...
          <Modal.Body className="m-3">
            <Form id="create-department-form" onSubmit={handleSubmit}>
              ...
              <Form.Group className="mb-3">
                <Form.Label className="required">Tên phòng thí nghiệm</Form.Label>
                <Form.Control
                  size="md"
                  type="text"
                  name="name"
              ...
              {(touched.departmentsId || touched.subjectsId) && !values.departmentsId && !values.subjectsId && (
                <div className="text-danger">Bạn phải chọn ít nhất một khoa hoặc bộ môn.</div>
              )}

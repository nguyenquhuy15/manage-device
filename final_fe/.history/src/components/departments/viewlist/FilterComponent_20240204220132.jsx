...
const FilterComponent = (props) => {
    ...
    const validationForm = Yup.object({
        ...
        minMember: Yup.number()
            ...
            .test('minMemberGreaterThanMaxMember', 'Must be less than max member',
                (minMember, obj) => {
                    let maxMember = obj.parent.maxMember;
                    if (minMember && maxMember && minMember > 0 && maxMember > 0 && minMember > maxMember) {
                        return false;
                    }
                    return true;
                }),
        maxMember: Yup.number()
            .positive('Must be greater than 0')
            .integer('Must be integer')
            .test('maxMemberLessThanMinMember', 'Must be greater than min member',
                (maxMember, obj) => {
                    let minMember = obj.parent.minMember;
                    if (minMember && maxMember && minMember > 0 && maxMember > 0 && maxMember < minMember) {
                        return false;
                    }
                    return true;
                }),
                                    ...
                                    <Col md={6}>
                                        <Form.Group>
                                            <Form.Control
                                                ...
                                                isInvalid={Boolean(touched.minMember && errors.minMember)}
                                                onBlur={handleBlur}
                                                onChange={handleChange} />
                                            {!!touched.minMember && (
                                                <Form.Control.Feedback type="invalid">
                                    ...
                                    <Col md={6}>
                                        <Form.Group>
                                            <Form.Control
                                                ...
                                                isInvalid={Boolean(touched.maxMember && errors.maxMember)}
                                                onBlur={handleBlur}
                                                onChange={handleChange} />
                                            {!!touched.maxMember && (
                                                <Form.Control.Feedback type="invalid">

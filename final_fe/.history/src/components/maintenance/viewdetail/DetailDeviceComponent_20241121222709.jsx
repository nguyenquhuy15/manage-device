...
const DetailDeviceComponent = (props) => {
  ...
  const getDetailDeviceInfo = async () => {
    try {
      const deviceInfo = await DeviceApi.getDetailMaintenance(deviceId);
  ...
  const onUpdateDeviceItem = useCallback(
    async (id, date, address, description, note, expense, status) => {
      // call api
      await DeviceApi.updateDeviceMaintenance(
        id,
        date,
        address,
        description,

...
class LoanAPI {
  ...
  getAll = ({ page, sortField, isAsc, search, loanDate, returnDate, status }) => {
    let url = this.url;
  ...
  removeDeviceInDetailLoan = (deviceId) => {
    return this.removeAllDevicesInDetailLoan([deviceId]);
  };

  removeAllDevicesInDetailLoan = (deviceIds) => {
    return AuthorApi.delete(`${this.url}/devices/${deviceIds.join(",")}`);
  };
}

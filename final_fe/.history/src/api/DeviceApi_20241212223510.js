...
class DeviceAPI {
  ...
  removeDeviceInMaintenance = (deviceId) => {
    return this.removeAllDevicesInMaintenance([deviceId]);
  };

  removeAllDevicesInMaintenance = (deviceIds) => {
    return AuthorApi.delete(`${this.url}/maintenance/${deviceIds.join(",")}`);
  };
  ...
  getAllDevicesByNoLoan = (page, search, sortField, isAsc, h) => {
    let url = `${this.url}/noLoan?page=${page}&size=${PAGING.SIZE}`;
